...
namespace Benaa.Api.Controllers
{
	...
	public class SchedualeController : ControllerBase
	{
		...
		[HttpGet("GetTeacherSceduales")]
		[ProducesResponseType(StatusCodes.Status200OK)]
		[ProducesResponseType(StatusCodes.Status400BadRequest)]
		[ProducesResponseType(StatusCodes.Status500InternalServerError)]
		public async Task<IActionResult> GetTeacherSceduales(string userId)
		{
			try
			{
				var result = await _scedualeService.GetTeacherSceduales(userId);
				if (result.IsError) { return BadRequest(result.ErrorsOrEmptyList); }
				return Ok(result.Value);
			}
			catch (Exception ex)
